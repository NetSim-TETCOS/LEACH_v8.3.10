	...
	NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	rreqOption->targetAddress = DSR_DEV_IP_COPY(target);
...
int fn_NetSim_DSR_ProcessRREQ(NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	if(!IP_COMPARE(rreq->targetAddress,DSR_DEV_IP(pstruEventDetails->nDeviceId)))
	{
	...
	else
	{
		...
		if(fn_NetSim_DSR_CheckIPinIPList(rreq->address,length,DSR_DEV_IP(pstruEventDetails->nDeviceId)))
		{
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					rreq->address[DSR_RREQ_LEN(rreq)] = DSR_DEV_IP_COPY(pstruEventDetails->nDeviceId);
					rreq->nOptDataLen +=4;
